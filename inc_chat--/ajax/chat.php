...
function fetchMessages(){
    ...
    $sql_compare_current_reg_pts   = "SELECT * from users_chat where  type_user = 'patients' ";
    $query_compare_current_reg_pts = mysqli_query($connection2, $sql_compare_current_reg_pts);
    $fetching_compare_current_reg_pts = mysqli_fetch_assoc($query_compare_current_reg_pts);
    $get_compare_current_reg_pts = $fetching_compare_current_reg_pts['current_reg'];
    $get_compare_room_pts = $fetching_compare_current_reg_pts['room'];
    $get_compare_user_pts = $fetching_compare_current_reg_pts['user_id'];


    $sql_compare_current_reg_dr   = "SELECT * from users_chat where  type_user = 'doctor' ";
    $query_compare_current_reg_dr = mysqli_query($connection2, $sql_compare_current_reg_dr);
    $fetching_compare_current_reg_dr = mysqli_fetch_assoc($query_compare_current_reg_dr);
    $get_compare_current_reg_dr = $fetching_compare_current_reg_dr['current_reg'];
    $get_compare_room_dr = $fetching_compare_current_reg_dr['room'];
    $get_compare_user_dr = $fetching_compare_current_reg_dr['user_id'];

    
    $sql_select_chats = "SELECT chat.messages, chat.message_at, chat.img_sending, 
    users_chat.username, users_chat.user_id , users_chat.type_user, users_chat.room
    ...
    $query_select_chats = mysqli_query($connection2, $sql_select_chats);
          ...
          while($rows_chats = mysqli_fetch_assoc($query_select_chats)){
                ...
                $room_number = $rows_chats['room'];
              }

              if($type_user === 'patients' 
              AND $get_compare_current_reg_pts == $get_compare_current_reg_dr 
              AND $get_compare_user_dr == $get_compare_user_pts){
                ...
                echo "<div class='message-data align-right'>";
                    ...
                    <span style='color:red'>$type_user</span>
                     . $messages_chat_at 10:00 pm, Today </span>";
                        echo "<span name='uname'>" .$messages_username . "</span>";
                          ...
                          // if(empty($messages_chat) === true){
                                  ...
                                  // echo "images/imgs/_".date('y-m-d')."";
                              if($messages_chat_img_sending == "images/imgs/_".date('y-m-d').""){
